...
shoppingRouter.get("/lists/by-token/:token", async (req, res) => {
  const found = await db
    ...
    .limit(1);
  if (found.length === 0) return res.status(404).json({ error: "Lista nao encontrada" });
...
shoppingRouter.post("/lists/by-token/:token/items", async (req, res) => {
  ...
  const list = await db
    ...
    .where(eq(schema.shoppingLists.publicToken, req.params.token))
    .limit(1);
  if (list.length === 0) return res.status(404).json({ error: "Lista nao encontrada" });
  if (list[0].status !== "open") return res.status(400).json({ error: "Lista ja fechada" });
...
shoppingRouter.get("/lists/by-token/:token/qrcode", async (req, res) => {
  const found = await db
    ...
    .limit(1);
  if (found.length === 0) return res.status(404).json({ error: "Lista nao encontrada" });
  const baseUrl = (req.headers["x-app-origin"] as string) || `${req.protocol}://${req.get("host")}`;
