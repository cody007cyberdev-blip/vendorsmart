...
productsRouter.get("/", requireAuth, async (req, res) => {
  ...
  if (supplierId) conditions.push(eq(schema.products.supplierId, supplierId));
  if (status) conditions.push(eq(schema.products.status, status as "active" | "discontinued" | "archived"));
  if (stockState === "out") {
    conditions.push(sql`${schema.products.currentStock} = 0`);
  } else if (stockState === "low") {
    conditions.push(
      sql`${schema.products.currentStock} > 0 AND ${schema.products.currentStock} <= ${schema.products.reorderPoint}`,
    );
  } else if (stockState === "ok") {
    conditions.push(
      sql`${schema.products.currentStock} > ${schema.products.reorderPoint}`,
    );
  }
...
productsRouter.get("/:id", requireAuth, async (req, res) => {
  const found = await db
    ...
    .limit(1);
  if (found.length === 0) return res.status(404).json({ error: "Nao encontrado" });
...
productsRouter.put("/:id", requireRole("admin", "manager"), async (req, res) => {
  ...
  const updated = await db
    ...
    .where(eq(schema.products.id, req.params.id))
    .limit(1);
  if (updated.length === 0) return res.status(404).json({ error: "Nao encontrado" });
