...
async function fetchReportRows(
  type: z.infer<typeof generateSchema>["type"],
...
async function toXlsxBuffer(headers: string[], rows: (string | number)[][]) {
...
reportsRouter.get("/:id/download", requireAuth, async (req, res) => {
  ...
  if (filename.endsWith(".csv")) res.setHeader("Content-Type", "text/csv; charset=utf-8");
