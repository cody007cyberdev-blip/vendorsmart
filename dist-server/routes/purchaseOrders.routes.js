...
purchaseOrdersRouter.post("/", requireRole("admin", "manager"), async (req, res) => {
    ...
    const totalCost = parsed.data.items.reduce((sum, i) => sum + i.quantityOrdered * i.unitCost, 0);
...
purchaseOrdersRouter.put("/:id/status", requireRole("admin", "manager"), async (req, res) => {
...
purchaseOrdersRouter.post("/:id/receive", requireRole("admin", "manager"), async (req, res) => {
    ...
    const order = await db
        ...
        .limit(1);
    if (order.length === 0)
        return res.status(404).json({ error: "Ordem nao encontrada" });
